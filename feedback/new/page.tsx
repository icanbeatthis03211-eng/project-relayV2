...
export default function NewFeedbackPage() {
  ...
  async function handleSubmit() {
    ...
    // 과제 파일을 함께 첨부했다면, 피드백을 저장하기 전에 먼저 업로드해요.
    // 업로드에 실패하면 피드백도 저장하지 않고 버튼 하나로 함께 실패/성공하게 해요.
    let attachment: { path: string; name: string; size: number } | null = null;
    if (attachmentFile) {
      const { data: uploaded, error: uploadErr } = await uploadFeedbackAttachment(
        userId,
        attachmentFile
      );
      if (uploadErr || !uploaded) {
        setSubmitting(false);
        setError(uploadErr ?? "과제 파일 업로드에 실패했어요.");
        return;
      }
      attachment = { path: uploaded.path, name: attachmentFile.name, size: attachmentFile.size };
    }
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      <Card>
        <p className="text-lg font-semibold text-gray-900 mb-1">과제 파일 (선택)</p>
        <p className="text-xs text-gray-400 mb-3">
          이 피드백과 관련된 과제 파일이 있다면 함께 첨부해서 저장할 수 있어요.
        </p>
        <input
          type="file"
        ...
        {attachmentFile && (
          <div className="flex items-center justify-between mt-3 text-xs text-gray-500">
            ...
            <button
              ...
              className="shrink-0 text-gray-400 hover:text-red-600 ml-2"
            >
              제거
            </button>
          </div>
